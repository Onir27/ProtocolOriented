...
class UsersTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        Post.fetch(with:PostRequest.getPosts, onSuccess: { [weak self] result in
            if case let MappingResult.asArray(userArray) = result {
                self?.dataSource = userArray
                DispatchQueue.main.async {
                    self?.tableView.reloadData()
                }
            }
            if case let MappingResult.asSelf(post) = result {
                print(post.title!)
            }
